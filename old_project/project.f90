...
!-----------------------------------------------------------------------------!
program ionization ! 'ionization' is the main program (fortran notation?)

use stuff       !'stuff' is a separate module where variables are defined
implicit none   !should return an error when undefined variables are used
external func   !'func' is at the bottom of this program... this is where a 
                !  shitload of calculation are done and n_Elec is provided.
                !  for every single layer.  
!double precision :: func, zbrent
real*8 :: func,zbrent

!Open files for writing data (one for each required plot)
open(unit=11,file='plot1.txt')!Main physical quantities: (physical depth, T,
                                !  opacity, and rho; all vs. optical depth
                                !  (tau))
open(unit=12,file='plot2.txt')!Pressures 
open(unit=13,file='plot3.txt')!Ionization fractions
open(unit=14,file='plot4.txt')!Electron density and partial electron density
open(unit=15,file='plot5.txt')!H excitation fractions


!Open existing file containing...?
open(unit=1,file='mihalas.txt',status='old',action='read')!from course site
read(1,*) opd_in        !optical depth??
read(1,*) q_in          !q????
close(unit=1)           !close file; no longer needed

!Read in values from opacity table
open(unit=1,file='opacity.txt',status='old',action='read')!from course site
read(1,*) opacity_data          ! crap-ton of numbers
R_table=opacity_data(2:20,1)    !first row
T_table=opacity_data(1,2:71)    !first column
X_table=opacity_data(2:20,2:71) !rest of table
close(unit=1)                   !close file; no longer needed

!Set up tau array
opd(1)=opd_min  !double-check that fortran indices start at 1, not 0...
                ! Actually I'm pretty sure it's 0
d_opd=1.0d-3    !delta optical depth
counter=1       !to keep track of tau array indices
...
g=grav_const*mass/(radius**2)           !star's accel. due to gravity
P_gas(1)=10**(1.6+(2/3)*(log(g)-4.0))   !gas pressure
temp(1)=Teff
depth(1)=0.0
counter=1
bool=1
